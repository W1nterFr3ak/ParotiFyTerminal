...
def parrotify(path):
	print("Please wait. Modifying  :"+ path)
	t = open(path , 'w+')
...
def reversify(old, new, user=os.getlogin()):
	#old = '/home/' + user + '/.bashrc'
	#new = '/home/' + user + '/.bashrcold'
	if  os.path.isfile(new):
		os.system(f'mv {new} {old} ')
		print("Please sip your coffee as winter works his magic -*-*-")
		time.sleep(8)
		print('\n\n')
		os.system('echo "\\e[1;32m[-] Reversing Parrotify was succesful, close terminal and reopen  \\e[0m"')
		
	else:
		os.system(f"cp /etc/skel/.bashrc {old}")
		print("Looks like you never used me to parrotify or maybe my brain froze \
		\nBut let me be cautious restoring from /etc/skel  -*-*-")
		time.sleep(8)
		print("\n\n")
		os.system('echo "\\e[1;32m[-] Reversing Parrotify was succesful, close terminal and reopen  \\e[0m"')
